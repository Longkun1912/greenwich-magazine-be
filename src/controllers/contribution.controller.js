...
exports.createContribution = async (req, res) => {
    try {
        const result = await contributionService.createContribution(req.body, req.file);
...
exports.updateContribution = async (req, res) => {
    try {
        const result = await contributionService.updateContribution(req.params.id, req.body);
        res.json(result);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
